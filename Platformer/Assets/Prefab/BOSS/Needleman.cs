...
public class Needleman : MonoBehaviour {
	...
	void Shoot(){
		GameObject shot = Instantiate(spike) as GameObject;
		shot.transform.position = this.transform.position;
		PE_Obj peo_shot = shot.GetComponent<PE_Obj>();
		float x_vec = player.transform.position.x - this.transform.position.x;
		float y_vec = player.transform.position.y - this.transform.position.y;


		peo_shot.vel.x = x_vec;
		peo_shot.vel.y = y_vec;
//		float step = 1f;
//		shot.transform.position = Vector3.MoveTowards (this.transform.position, player.transform.position, step);
	}
